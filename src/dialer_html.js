      ...
      <div class="header-bar">
        ...
        <div style="position: relative; display: inline-block;">
          ...
          <div class="header-dropdown" id="headerDropdown" style="display: none;">
            ...
            <button class="dropdown-item copy-limit-sub" id="toggleThreshold15">  15次限制: ✓</button>
  ...
  (function(){
    ...
    function parsePhoneContactsFromRawText(text) {
      ...
      function isNoteLike(str) {
        ...
        if (str.length > 10 && !/有限|集团|公司|厂|店|中心|商行|工作室|学校|医院/i.test(str)) {
          return true;
        }
        return false;
      }
    ...
    function initCustViewer(){
      ...
      if (aiFundBtn) {
        aiFundBtn.addEventListener('click', function() {
          if (!confirm('即将使用 AI 扫描所有客户记录，检查公积金字段中是否误存了公司名称。\\n\\n' +
              'AI 将逐条判断：\\n' +
              '1. 如果 fund 值是公司名称 → 自动移动到「单位」字段并清空公积金\\n' +
              '2. 如果 fund 值是乱码/备注 → 直接清空公积金\\n' +
              '3. 不确定的条目将跳过，不做修改\\n\\n' +
              '⚠️ 仅修改「公积金」和「单位」字段，不影响其他数据。\\n' +
              '⚠️ 所有修改按行进行，不会交叉合并数据。\\n\\n' +
              '确认开始 AI 扫描修正？')) {
            return;
          }
