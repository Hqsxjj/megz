  ...
  window.addEventListener('beforeunload',()=>{
    ...
    if (path === '/api/ocr' && request.method === 'POST') {
      try {
        ...
        const ocrMode = body.mode || 'single'; // 'single' = 单客户提取, 'bulk' = 批量联系人提取
        if (!imageBase64) {
                  ...
                  content: [
                    {
                      type: 'text',
                      text: ocrMode === 'bulk'
                        ? '你是一个专业的信息提取助手。请仔细识别这张图片中出现的所有联系人信息。\n\n图片可能是：微信/QQ聊天截图、通讯录截图、表格、名片、文字列表、Excel截图等任意格式。\n\n提取规则：\n1. 姓名：中文姓名2-4个字，或英文姓名。如果是聊天截图，关注发送名片或介绍时提到的名字。\n2. 电话：手机号（通常11位，1开头）或座机号（含区号）。忽略明显不是电话号码的数字串（如金额、日期、ID）。\n3. 公司/单位：公司全称或简称，通常在姓名或电话附近。\n4. 备注：其他有价值的信息（职位、地址、业务需求等），不要写无关的聊天内容。\n\n重要：\n- 仔细扫描图片每一处文字，不要遗漏角落或小字。\n- 即使某些字段缺失，只要有姓名或电话之一就收录。\n- 如果同一人出现多次，只保留信息最全的一条。\n- rawText中完整转录图片所有文字，保持原始排版，用于本地二次解析。\n\n输出纯JSON（禁止markdown代码块，禁止```json包裹）：\n{\n  "contacts": [\n    {"name": "张三", "phone": "13800138000", "company": "某某科技有限公司", "note": "销售经理"}\n  ],\n  "rawText": "图片完整文字转录..."\n}'
