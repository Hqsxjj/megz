              ...
              content: [
                {
                  type: 'text',
                  text: ocrMode === 'bulk'
                    ? '你是一个OCR文字识别助手。请识别这张图片中的所有文字，逐行完整转录，不要遗漏任何内容。\n\n要求：\n1. rawText: 逐行转录图片中所有可见文字，包括表头、姓名、电话、公司名、地址、备注等一切内容\n2. contacts: 从文字中提取的每条数据记录，包含以下字段（无法识别的为空字符串）：\n   - name: 姓名（中文姓名2-4字）\n   - phone: 电话号码（11位手机号或座机号）\n   - company: 公司/单位名称\n   - note: 其他列的内容（如备注、地址、职务、公积金等，合并为一个字符串）\n3. 如果图片是表格，逐行提取每个单元格\n4. 如果是聊天截图，提取所有可见消息\n\n输出纯JSON（不要markdown代码块）：\n{"rawText":"所有文字内容","contacts":[{"name":"张三","phone":"13800138000","company":"某某科技","note":"备注内容"}]}'
