    ...
    if (path === '/api/ocr/correct' && request.method === 'POST') {
      try {
        ...
        const systemPrompt = '你是一个 OCR 文本修正专家。下面的文本是从图片中通过 OCR 引擎识别出来的，典型错误包括：\n' +
          ...
          '4. 换行和空格导致文本断裂或错位\n\n' +
          '【文本排版特征说明】：\n' +
          '源文本是按行排版的联系人表格，每一行典型的字段顺序通常为：\n' +
          '【姓/姓名】 【手机号】 【公积金/金额】 【公司/单位名称】\n' +
          ...
          '4. 按照规则提取所有联系人信息。\n\n' +
          '【提取规则】：\n' +
          '- name: 中文姓名（1-4个汉字）。注意：第一列通常为单字姓氏（如“青”、“温”、“朱”、“刘”、“张”、“葛”、“莫”等），即使只有一个汉字，也是联系人的姓名/姓氏，请务必完整提取并填充到 name 字段，绝对不要忽略、丢弃或擅自补全为其他字。\n' +
          '- phone: 11位纯数字手机号（1开头）\n' +
          '- company: 公司/单位名称\n' +
    ...
    if (path === '/api/ocr/categorize' && request.method === 'POST') {
      ...
      try {
        ...
        const systemPrompt = '你是一个通讯录数据清洗与智能分类专家。输入是一个包含从图片表格中通过本地 OCR 识别初步对齐的联系人 JSON 数组。\n' +
          ...
          '2. 本应是“姓名”的信息被误归类为了“公司”或“备注”（姓名通常为1-4个中文字，绝对不能是公司或单位名字。注意：允许并常有单字姓氏作为姓名的客情，例如单字姓“青”、“温”、“朱”、“刘”、“张”、“葛”、“莫”等，必须作为 name 保留，不可漏掉）。\n' +
          ...
          '【整理规则】：\n' +
          '- name: 姓名/姓氏（必须是人名/姓氏，通常为1-4个汉字，即使是单字姓氏如“温”、“青”、“朱”、“刘”等也必须作为 name 保留。不能包含机构、集团、公司、实验室、研究所、学校等单位名称）。\n' +
          '- phone: 保持原样不变。\n' +
