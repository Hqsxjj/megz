...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .notify-bar { position: fixed; top: 0; left: 0; right: 0; background: var(--accent-intent); color: #fff; padding: 14px 20px; font-size: 0.9rem; font-weight: 700; z-index: 10000; transform: translateY(-100%); transition: transform 0.35s cubic-bezier(0.34,1.56,0.64,1); text-align: center; box-shadow: 0 4px 20px rgba(0,0,0,0.2); cursor: pointer; }
    .notify-bar.show { transform: translateY(0); }
    .notify-bar .notify-close { position: absolute; right: 16px; top: 50%; transform: translateY(-50%); font-size: 1.2rem; opacity: 0.7; }
      ...
      <div class="right-area">
        ...
        <div class="card">
          <div class="todo-section">
            ...
            <div class="todo-input-row"><input type="text" class="todo-input" id="todayTodoInput" placeholder="添加今日待办..." autocomplete="off"><input type="time" class="todo-input" id="todayRemindTime" style="flex:0 0 100px;padding:8px 4px;font-size:0.7rem;"><button class="todo-add-btn" id="addTodayTodoBtn">+ 添加</button></div>
          </div>
        </div>
        <div class="card">
          <div class="todo-section">
            ...
            <div class="todo-input-row"><input type="text" class="todo-input" id="todoInput" placeholder="添加明日待办..." autocomplete="off"><input type="time" class="todo-input" id="tomorrowRemindTime" style="flex:0 0 100px;padding:8px 4px;font-size:0.7rem;"><button class="todo-add-btn" id="addTodoBtn">+ 添加</button></div>
          </div>
        </div>
      </div>
    </div>
...
(function(){
  ...
  function checkReminders(){
    ...
    const today=getTodayStr();
    const tt=loadTodos(TODAY_TODO_K);
    const all=[...tt.map(t=>({...t,list:'today'}))];
    for(const t of all){
  ...
  function checkReminders(){
    ...
    const hm=String(now.getHours()).padStart(2,'0')+':'+String(now.getMinutes()).padStart(2,'0');
    const tt=loadTodos(TODAY_TODO_K);
    const tm=loadTodos(TOMORROW_TODO_K);
    const all=[...tt.map(t=>({...t,list:'today'})),...tm.map(t=>({...t,list:'tomorrow'}))];
    for(const t of all){
      if(!t.remind||t.remind!==hm)continue;
      const key=t.list+'_'+t.text+'_'+t.remind;
      if(lastNotified[key])continue;
